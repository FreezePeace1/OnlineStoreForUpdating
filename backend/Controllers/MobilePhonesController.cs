...
namespace backend.Controllers
{
    ...
    public class MobilePhonesController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<MobilePhones>>> GetMobilePhones()
        {
            ...
            var projectImages = Directory.GetFiles("./images").Select(Path.GetFileName);

            var dbImages = _context.MobilePhones.ToList();

            var matchedImAGES = dbImages.Where(image => projectImages.Contains(image.images));

            return Ok(matchedImAGES);
        }
